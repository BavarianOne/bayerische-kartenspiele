...
'''
  ...
  if(state.seqState >= 1 && state.seqState <= 5){
    ...
    // SIDE VIEW during atmospheric entry
    if(state.seqState === 1){
...
'''
    ...
    content = content.replace('state.seqState = 6;', 'state.seqState = 7;')
    content = content.replace('state.seqState = 0;', 'state.seqState = 0;')
    ...
    print(f"Dateigröße: {len(content)} -> {len(content) - len(original_content)} Bytes Differenz")
...
if __name__ == '__main__':
    ...
    original_content = read_file(path='/root/bayerische-kartenspiele/space-explorer-3d.html')['content']
    main()
